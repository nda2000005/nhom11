...
class DashboardModel {
    ...
    // SỬA ĐỔI TẠI ĐÂY: Hàm lấy đơn hàng có JOIN sang bảng customers
    public function getRecentOrders($limit = 5) {
        try {
            // Kiểm tra các cột trong bảng orders để xác định khóa ngoại
            $order_cols = $this->db->query("DESCRIBE orders")->fetchAll(PDO::FETCH_COLUMN);
            $fk = in_array('customer_id', $order_cols) ? 'customer_id' : 'user_id';
            
            // Lấy cột tên từ bảng customers
            ...
            // Dự phòng lỗi: trả về danh sách không có tên khách nếu join thất bại
